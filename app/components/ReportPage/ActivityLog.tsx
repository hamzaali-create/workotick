...
import { Logs, LogsKeys } from "@/type/ActivityLog";

export default function ActivityLog({ date }: any) {
  ...
  const getTimeLogs = useCallback(async () => {
    try {
      ...
      const { data } = await Api.Post({
        url: `/organization/${activeOrganization?.id}/my-report/activity`,
        postData: {
  ...
  return (
    <LazyLoad loading={loading} loader={<ActivityLogLoader />}>
      <div className="bg-white px-5 pt-5 my-3 rounded-md py-4 flex-grow">
        <p className="text-xl font-medium mt-1">Activity log</p>
        <div className="grid grid-cols-2 gap-x-2 xl:gap-x-2 gap-y-5 rounded-md my-4">
          <div className="col-span-1 p-2 xl:p-3 bg-[#F3F3F3] rounded-md">
            ...
            <p className="font-bold  xl:text-xl text-[#263238]">
          ...
          {(Object.keys(logs) as (keyof Logs)[])
            .filter((key) => !["clock_in", "clock_out"].includes(key))
            .map((key) => (
              <div
                className="col-span-1 p-2 xl:p-3 bg-[#F3F3F3] rounded-md"
                key={key}
              >
                <p className="text-gray-500 font-semibold xl:my-2 capitalize">
