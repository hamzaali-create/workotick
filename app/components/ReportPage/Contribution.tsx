  ...
  const getContributions = useCallback(async () => {
    try {
      const { data } = await Api.Post({
        url: `organization/${activeOrganization.id}/team-report/time-summary/contribution`,
        postData: {
    ...
    <div>
      ...
      >
        <div className="grid grid-cols-3  pb-3  mt-8 items-center justify-between mx-5">
          <h2 className="col-span-3 lg:col-span-1 pb-3 text-lg font-semibold w-full">
          ...
          <div className="col-span-3 lg:col-span-1 py-3  items-end flex flex-col">
            <h2 className="text-lg font-semibold">
              Today, {dayjs(date).format("DD, MMMM")}
            </h2>
            <div className="flex items-center gap-2 mt-3">
              <p>Total Hours :</p>
              <p className="text-[#32186F] bg-[#9854CB] bg-opacity-30 rounded-full px-3 font-medium text-md">
                {totalTime}
              </p>
            </div>
          </div>
